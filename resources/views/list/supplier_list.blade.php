...
<div class="content-wrapper">
        ...
        <div class="card border-0 shadow-sm">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-lg-5">
                            <div class="input-group">
                                ...
                                <input type="text" name="search" class="form-control" placeholder="સપ્લાયર શોધો..."
                                    value="{{ $search }}">
                            </div>
                        </div>
                            ...
                            <tr data-row-id="{{ $supplier->id }}">
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <img src="https://ui-avatars.com/api/?name={{ urlencode($supplier->name) }}"
                                ...
                                <td class="text-center">
                                    ...
                                    <a href="{{ route('supplier.purchases', $supplier->id) }}"
                                        class="btn btn-sm btn-outline-primary" title="ખરીદીઓ જુઓ">
                                        <i class="bx bx-show"></i>
                                    </a>

                                    <button class="btn btn-sm btn-outline-warning edit-supplier-btn"
                                        data-id="{{ $supplier->id }}" data-name="{{ $supplier->name }}"
                                        data-address="{{ $supplier->address }}" data-bs-toggle="modal"
                                        data-bs-target="#editSupplierModal" title="એડિટ કરો">
                                        <i class="bx bx-edit"></i>
                                    </button>

                                    <button class="btn btn-sm btn-outline-danger delete-supplier-btn"
                                        data-id="{{ $supplier->id }}"
                                        data-balance="{{ $supplier->purchases_sum_balance_amount ?? 0 }}"
                                        title="કાઢી નાખો">
            ...
            <div class="card-footer bg-white">
                ...
                <div class="d-flex flex-wrap justify-content-between align-items-center">
                    ...
                    <small class="text-muted">
                        {{ $suppliers->total() }} સપ્લાયરોમાંથી
                        {{ $suppliers->total() ? $suppliers->firstItem() : 0 }} થી {{ $suppliers->lastItem() ?? 0 }}
                    ...
                    <nav>
                        ...
                        <ul class="pagination pagination-sm mb-0">
                            ...
                            <li class="page-item {{ $suppliers->onFirstPage() ? 'disabled' : '' }}">
                                <a class="page-link"
                                    href="{{ $suppliers->onFirstPage() ? '#' : $suppliers->previousPageUrl() }}">પાછળ</a>
                            </li>
                            ...
                            <li class="page-item {{ !$suppliers->hasMorePages() ? 'disabled' : '' }}">
                                <a class="page-link"
                                    href="{{ $suppliers->hasMorePages() ? $suppliers->nextPageUrl() : '#' }}">આગળ</a>
                            </li>
                    ...
                    const response = await fetch("{{ route('suppliers.store') }}", {
                        ...
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector(
                                '#supplierForm input[name="_token"]').value,
    ...
    <script>
        ...
        document.getElementById('editSupplierForm').addEventListener('submit', async function(e) {
            ...
            const updateBtn =
                document.getElementById('updateSupplierBtn');

            const updateText =
                document.getElementById('updateSupplierText');
    ...
    <script>
        document.addEventListener('click', function(e) {
            ...
            GlassToast.confirm(
                'સપ્લાયર કાઢી નાખો', confirmMessage, async function() {
